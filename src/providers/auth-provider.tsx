...
function saveLoginTime(d: Date) { try { localStorage.setItem(LOGIN_TIME_KEY, d.toISOString()); } catch {} }
function readLoginTime(): Date | null { try { const v = localStorage.getItem(LOGIN_TIME_KEY); return v ? new Date(v) : null; } catch { return null; } }
function clearLoginTime() { try { localStorage.removeItem(LOGIN_TIME_KEY); } catch {} }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateName = useCallback(async (name: string) => {
    ...
    const res = await apiFetch("/api/auth/me", { method: "PATCH", body: JSON.stringify({ name: trimmed }) });
  ...
  return (
    <AuthContext.Provider value={{ currentUser, isReady, loginTime, signup, login, logout, updateName }}>
      {children}
